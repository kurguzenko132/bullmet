...
export function AdminHomepageClient({ initialSettings, reviews }: { initialSettings: HomeControlSettings; reviews: AdminReview[] }) {
  ...
  function patchSection(section: keyof Pick<HomeControlSettings, 'directionsSection' | 'productsSection' | 'collectionsSection' | 'productionSection' | 'stepsSection' | 'gallerySection' | 'reviewsSection' | 'faqSection' | 'cta'>, patch: Record<string, unknown>) {
  ...
  return <div className="homepage-cms">
    ...
    {tab === 'structure' && <>
      ...
      <section className="homepage-cms-split homepage-cms-lower"><div className="homepage-cms-card"><div className="homepage-cms-card-title"><div><h2>Популярные товары</h2><small>Товары отображаются из каталога</small></div><Visibility visible={settings.productsSection.enabled} onClick={() => patchSection('productsSection', { enabled: !settings.productsSection.enabled })} /></div><div className="homepage-cms-fields two"><Field label="Заголовок" value={settings.productsSection.title} onChange={(value) => patchSection('productsSection', { title: value })} /><Field label="Количество товаров" value={String(settings.productsSection.limit)} onChange={(value) => patchSection('productsSection', { limit: Math.max(1, Math.min(8, Number(value) || 4)) })} /><Field label="Кнопка" value={settings.productsSection.buttonLabel} onChange={(value) => patchSection('productsSection', { buttonLabel: value })} /><Field label="Ссылка" value={settings.productsSection.buttonHref} onChange={(value) => patchSection('productsSection', { buttonHref: value })} /></div></div><div className="homepage-cms-card"><div className="homepage-cms-card-title"><div><h2>Индивидуальное исполнение</h2><small>Размер, цвет и варианты исполнения модели</small></div><Visibility visible={settings.cta.enabled} onClick={() => patchSection('cta', { enabled: !settings.cta.enabled })} /></div><div className="homepage-cms-fields two"><Field label="Eyebrow" value={settings.cta.eyebrow} onChange={(value) => patchSection('cta', { eyebrow: value })} /><Field label="Заголовок" value={settings.cta.title} onChange={(value) => patchSection('cta', { title: value })} /><Field label="Описание" value={settings.cta.text} rows={4} onChange={(value) => patchSection('cta', { text: value })} /><AdminImagePicker label="Изображение" value={settings.cta.image} onChange={(value) => patchSection('cta', { image: value })} /><Field label="Основная кнопка" value={settings.cta.primaryLabel} onChange={(value) => patchSection('cta', { primaryLabel: value })} /><Field label="Ссылка основной кнопки" value={settings.cta.primaryHref} onChange={(value) => patchSection('cta', { primaryHref: value })} /><Field label="Вторая кнопка" value={settings.cta.secondaryLabel} onChange={(value) => patchSection('cta', { secondaryLabel: value })} /><Field label="Ссылка второй кнопки" value={settings.cta.secondaryHref} onChange={(value) => patchSection('cta', { secondaryHref: value })} /></div></div></section>
      <section className="homepage-cms-card"><div className="homepage-cms-card-title"><div><h2>Преимущества индивидуального исполнения</h2><small>До трёх коротких пунктов</small></div></div><div className="homepage-cms-sort-list">{sortItems(settings.cta.benefits).map((item) => <article className="homepage-cms-sort-row" key={item.id}><Visibility visible={item.visible} onClick={() => change((current) => ({ ...current, cta: { ...current.cta, benefits: updateItem(current.cta.benefits, item.id, { visible: !item.visible }) } }))} /><span><input value={item.title} onChange={(event) => change((current) => ({ ...current, cta: { ...current.cta, benefits: updateItem(current.cta.benefits, item.id, { title: event.target.value }) } }))} /><input value={item.desc} onChange={(event) => change((current) => ({ ...current, cta: { ...current.cta, benefits: updateItem(current.cta.benefits, item.id, { desc: event.target.value }) } }))} /></span><button type="button" onClick={() => change((current) => ({ ...current, cta: { ...current.cta, benefits: moveItem(current.cta.benefits, item.id, -1) } }))}><ArrowUp size={15} /></button><button type="button" onClick={() => change((current) => ({ ...current, cta: { ...current.cta, benefits: moveItem(current.cta.benefits, item.id, 1) } }))}><ArrowDown size={15} /></button></article>)}</div></section>
      <section className="homepage-cms-card">
        <div className="homepage-cms-card-title"><div><h2>Коллекции / Подберите часы под интерьер</h2><small>Карточки ведут в каталог с готовой подборкой</small></div><Visibility visible={settings.collectionsSection.enabled} onClick={() => patchSection('collectionsSection', { enabled: !settings.collectionsSection.enabled })} /></div>
        <div className="homepage-cms-fields two"><Field label="Eyebrow" value={settings.collectionsSection.eyebrow} onChange={(value) => patchSection('collectionsSection', { eyebrow: value })} /><Field label="Заголовок" value={settings.collectionsSection.title} onChange={(value) => patchSection('collectionsSection', { title: value })} /><Field label="Текст ссылки" value={settings.collectionsSection.buttonLabel} onChange={(value) => patchSection('collectionsSection', { buttonLabel: value })} /><Field label="Ссылка" value={settings.collectionsSection.buttonHref} onChange={(value) => patchSection('collectionsSection', { buttonHref: value })} /></div>
        <div className="homepage-cms-category-list">{sortItems(settings.collections).map((item) => <article key={item.id}><img src={item.img} alt="" /><div><input value={item.title} onChange={(event) => change((current) => ({ ...current, collections: updateItem(current.collections, item.id, { title: event.target.value }) }))} /><input value={item.description} onChange={(event) => change((current) => ({ ...current, collections: updateItem(current.collections, item.id, { description: event.target.value }) }))} /><input value={item.href} onChange={(event) => change((current) => ({ ...current, collections: updateItem(current.collections, item.id, { href: event.target.value }) }))} /></div><Visibility visible={item.visible} onClick={() => change((current) => ({ ...current, collections: updateItem(current.collections, item.id, { visible: !item.visible }) }))} /><AdminImagePicker label="" value={item.img} onChange={(value) => change((current) => ({ ...current, collections: updateItem(current.collections, item.id, { img: value }) }))} /></article>)}</div>
      </section>
      <section className="homepage-cms-card">
      ...
      <section className="homepage-cms-card">
        <div className="homepage-cms-card-title"><div><h2>Частые вопросы</h2><small>На главной отображаются первые 6 видимых вопросов</small></div><Visibility visible={settings.faqSection.enabled} onClick={() => patchSection('faqSection', { enabled: !settings.faqSection.enabled })} /></div>
        <div className="homepage-cms-fields two"><Field label="Eyebrow" value={settings.faqSection.eyebrow} onChange={(value) => patchSection('faqSection', { eyebrow: value })} /><Field label="Заголовок" value={settings.faqSection.title} onChange={(value) => patchSection('faqSection', { title: value })} /><Field label="Описание" value={settings.faqSection.text} rows={3} onChange={(value) => patchSection('faqSection', { text: value })} /><AdminImagePicker label="Изображение" value={settings.faqSection.image} onChange={(value) => patchSection('faqSection', { image: value })} /></div>
        <div className="homepage-cms-sort-list">{sortItems(settings.faqItems).map((item) => <article className="homepage-cms-sort-row" key={item.id}><Visibility visible={item.visible} onClick={() => change((current) => ({ ...current, faqItems: updateItem(current.faqItems, item.id, { visible: !item.visible }) }))} /><span><input value={item.question} onChange={(event) => change((current) => ({ ...current, faqItems: updateItem(current.faqItems, item.id, { question: event.target.value }) }))} /><textarea value={item.answer} onChange={(event) => change((current) => ({ ...current, faqItems: updateItem(current.faqItems, item.id, { answer: event.target.value }) }))} /></span><button type="button" onClick={() => change((current) => ({ ...current, faqItems: moveItem(current.faqItems, item.id, -1) }))}><ArrowUp size={15} /></button><button type="button" onClick={() => change((current) => ({ ...current, faqItems: moveItem(current.faqItems, item.id, 1) }))}><ArrowDown size={15} /></button></article>)}</div>
      </section>
    </>}
