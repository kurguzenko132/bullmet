...
export async function getBackupDashboard(): Promise<BackupDashboard> {
  ...
  return { overview, records, settings: parseSettings(rawSettings), serviceReady, storageReady: serviceReady, schedulerReady: serviceReady && Boolean(process.env.BACKUP_CRON_SECRET), generatedAt: new Date().toISOString() };
}
...
async function getSnapshot(kind: BackupKind) {
  if (kind === 'media') return { generatedAt: new Date().toISOString(), media: await getAdminMediaFiles(), note: 'Снимок содержит реестр медиафайлов. Файлы Supabase Storage не копируются этим API.' };
  if (kind === 'database') return getExportData('all');
  return { data: await getExportData('all'), media: await getAdminMediaFiles(), note: 'Снимок данных и реестра медиа. Supabase Auth и физические файлы Storage требуют инфраструктурного восстановления.' };
}
...
export async function createBackup(input: { kind: BackupKind; comment?: string; createdBy?: string }) {
  ...
  let record: BackupRecord = { id, kind: input.kind, trigger: 'manual', status: 'creating', createdAt, includesDatabase: input.kind !== 'media', includesStorage: false, comment: input.comment?.trim() || undefined, createdBy: input.createdBy || 'Администратор', expiresAt: dateWithRetention(dashboard.settings.retentionCount), metadata: { version: 1, app: 'Bullmet', storageManifest: false, authIncluded: false } };
  let records = [record, ...dashboard.records]; await persistRecords(records); await logBackup('backup_create', record);
  try {
    await ensureBucket(); const snapshot = await getSnapshot(input.kind); const archive = JSON.stringify({ manifest: { version: 1, createdAt, kind: input.kind, includesDatabase: record.includesDatabase, includesStorage: false, authIncluded: false }, snapshot }); const bytes = Buffer.from(archive, 'utf8'); const checksum = createHash('sha256').update(bytes).digest('hex'); const storagePath = `${id}.json`;
