...
-- Coupon snapshots keep completed-order discount history immutable.
...
alter table public.profiles add column if not exists notification_preferences boolean not null default true;
...
-- CRM buyers: commercial contacts are separate from authentication profiles.
...
-- Coupons and their immutable usage history.
create table if not exists public.coupons (
  id uuid primary key default gen_random_uuid(), name text not null, code text not null unique,
  description text not null default '', type text not null check (type in ('percentage','fixed','free_shipping')),
  value numeric(12,2), max_discount numeric(12,2), status text not null default 'inactive' check (status in ('active','inactive','archived')),
  starts_at timestamptz, ends_at timestamptz, total_usage_limit integer, usage_limit_per_customer integer,
  minimum_order_amount numeric(12,2), first_order_only boolean not null default false, new_customers_only boolean not null default false,
  allow_discounted_products boolean not null default false, category_ids text[] not null default '{}', product_ids text[] not null default '{}',
...
create table if not exists public.coupon_usages (
  ...
  order_id text not null references public.orders(id) on delete restrict, customer_id uuid references public.profiles(id) on delete set null,
  customer_name text, customer_key text not null, order_total numeric(12,2) not null default 0, discount_amount numeric(12,2) not null default 0,
