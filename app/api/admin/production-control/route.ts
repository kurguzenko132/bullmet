...
export async function GET() {
  ...
  return NextResponse.json({ settings: withSiteSettingsRevision(mergeProductionControl(data?.value || defaultProductionControl), data?.updated_at), configured: true });
}
...
export async function POST(request: NextRequest) {
  ...
  const result = await saveSiteSettings(productionControlKey, settings, getSiteSettingsRevision(body?.settings || body));
