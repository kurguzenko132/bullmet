...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const warning = await logAdminActivity(request, { action: 'review_create', entity: 'product_reviews', entityId: data.id, after: data, payload: { source, status } });
    return NextResponse.json({ ok: true, review: data, warning: warning ? `Отзыв сохранён, но запись в журнал не добавлена: ${warning}` : undefined });
