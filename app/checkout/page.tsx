...
export default function CheckoutPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/orders', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ customer: form, delivery: form.delivery, comment: form.comment, items, accountEmail: form.email }) });
      const data = await response.json();
      if (!response.ok || !data.ok) throw new Error(data.message || 'Не удалось оформить заказ.');
      localStorage.setItem('bullmet_last_order', JSON.stringify({ id: data.id, total, items, customer: form, delivery: form.delivery, createdAt: new Date().toISOString() }));
