...
export default function CheckoutPage() {
  ...
  const selectedDelivery = deliveryOptions.find((option) => option.id === form.deliveryMethodId);
  const deliveryFree = selectedDelivery?.pricingType === 'free' || (selectedDelivery?.pricingType === 'fixed' && Boolean(selectedDelivery?.freeFromAmount || freeDeliveryFrom) && total >= Number(selectedDelivery?.freeFromAmount ?? freeDeliveryFrom));
  const deliveryPrice = selectedDelivery?.pricingType === 'fixed' && !deliveryFree ? Number(selectedDelivery.price || 0) : 0;
  const finalTotal = Math.max(0, total - Number(coupon?.discount || 0) + deliveryPrice);
  ...
  useEffect(() => {
    ...
    fetch('/api/delivery', { cache: 'no-store' })
      ...
      .then((data) => {
        const options = Array.isArray(data?.methods)
          ? data.methods.filter((item: DeliveryOption) => item?.enabled && !item.archived).sort((a: DeliveryOption, b: DeliveryOption) => a.order - b.order)
          : [];
